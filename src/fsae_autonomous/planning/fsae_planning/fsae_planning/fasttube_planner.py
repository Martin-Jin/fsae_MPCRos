...
class FasttubePlanner(Node):
    def __init__(self):
        super().__init__("fasttube")

        # parameters (from fsae_bringup/config/fsae_params.yaml)
        ...
        self.look_forward = self.get_parameter('look_forward').get_parameter_value().integer_value
        ...
        # subscribers
        self.subscription_left_cone_map = self.create_subscription(Track, '/fsae/slam/left_track',  self.left_cone_map_callback, 10)
        self.subscription_right_cone_map = self.create_subscription(Track, '/fsae/slam/right_track',  self.right_cone_map_callback, 10)
        self.create_subscription(Pose, "/fsae/slam/car_position", self.set_car_position, 10)   # car pose

        # publishers
        self.centerline_publisher = self.create_publisher(PoseArray, "/fsae/planning/selected_trajectory", 10)
    ...
    def loop(self) -> None:
        # self.get_logger().info(f"car pose received = {hasattr(self, 'car_pose')}")  # per-tick log spam (commented out)
        ...
        # NOTE: a hardcoded single-cone-colour DEBUG override used to live here — it
        # short-circuited real planning and published a crude 2-point hard turn when
        # only one boundary colour was visible. Removed so the real boundary logic
        # below runs; one-sided frames now return no trajectory, matching the other
        # planners. This guard must run BEFORE get_next_points: an empty boundary
        # (e.g. car_position arriving before the first SLAM track message at startup)
        # crashes np.argmin/np.linalg.norm on a zero-length array.
