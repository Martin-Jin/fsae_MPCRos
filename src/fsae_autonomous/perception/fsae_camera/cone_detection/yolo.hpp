...
KEY RESPONSIBILITIES:
  1. Data Structures: Defines BBox and BBoxInfo, the plain data containers that carry a
     ...
     raw output all the way through to cone_detection.cpp where they are deprojected to 3D.
...
DEPENDENCIES:
  ...
  - OpenCV: cv::Mat for receiving camera frames and internal image pre-processing.
  - cuda_utils.h / logging.h / utils.h: Project-local helpers for CUDA error checking, TensorRT logging,
    and image preprocessing (preprocess_img).
*/
...
of data that flows from Yolo::run() all the way to cone_detection.cpp where its box centre
is looked up in the depth image and deprojected into a 3D cone position.
*/
struct BBoxInfo {
    ...
    float prob; // Confidence score in [0, 1]. Yolo::run() only returns detections above the conf_threshold parameter.
};
...
resources, and exposes a clean three-method interface to the rest of the codebase:
  ...
  init():         Loads the pre-compiled .engine file from disk into GPU memory and allocates
                  ...
                  in the ZedLaunchNode constructor.

  run():          The hot path. Called for every incoming camera frame. Takes the BGRA camera
                  image (a cv::Mat decoded from the wrapper's ROS Image message), preprocesses it,
                  fires it through the GPU neural network, and parses the raw output tensor back
                  into a clean list of BBoxInfo detections. The result is immediately consumed by
                  cone_detection.cpp for depth lookup and deprojection.
*/
class Yolo {
public:
    Yolo();
    ...
    // Runs a full inference cycle on a single camera frame (BGRA cv::Mat) and returns a list of detected bounding boxes. This is called for every image callback in cone_detection.cpp.
