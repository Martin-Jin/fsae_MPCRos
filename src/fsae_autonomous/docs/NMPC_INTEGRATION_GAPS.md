...
| E7 | **Partially addressed, off by default.** The car veers sideways during a standing start, just before the controller corrects back. Root cause (reproduced offline, confirmed against a live 3 m/s flat-profile run): while the car is stationary the NMPC ramps its steering command to roughly -6.8 degrees over the ~1 s before the car physically moves, so it launches already turned. This is not a numerical defect. At `v_x = 0` steering has no effect (`r_kin = v_x*tan(d)/L = 0`, dynamic branch blended out by the already-fixed `F_yf`/`F_yr` `blend` scaling), but `_cost` is one scalar summed over the whole horizon and the rolled-out `v_x` profile leaves zero by stage 1 (`[0, 0.118, 0.294, 0.465, ...]`), so the optimiser pre-commits `U[0]` toward whatever helps the later, physically-active stages. Nothing in the objective distinguished "cannot act yet" from "will act soon". Mitigated by `nmpc_standstill_steer_damp_enabled` (**default False**), which scales stage 0's steering-effort weight by `nmpc_standstill_steer_r_scale` (20x) while the MEASURED speed is below `nmpc_standstill_speed` (0.5 m/s); stage 0 only, because every later stage can genuinely act, and `U[0]` is the only element shipped to the car. Measured on the reproduction: pre-load -6.71 -> -2.41 degrees at 20x, -1.18 at 50x, with the accel command unchanged. Flag-off is bit-identical to before. **Live result (2026-09-15)**: the damping suppresses the pre-load as designed (matched A/B on a flat 3 m/s profile: peak stationary steer 6.91 -> 2.48 deg at 20x, and 0.91 deg at 200x) and improved the launch on that map (yaw excursion 3.82 -> 2.81 deg, yaw rate 0.245 -> 0.185 rad/s, whole-run `|e_y|` mean 0.092 -> 0.075 m). On the REAL speed profile it did not: yaw excursion was 10.75 deg at 50x and 11.42 deg at 200x, i.e. eliminating the remaining pre-load changed the actual excursion by nothing. Cause: a hard release at `nmpc_standstill_speed` moved the excursion rather than removing it. Logged tick-by-tick, steering ran -1.8 to -12.9 deg over the six ticks immediately AFTER the release (~-2 deg/tick sustained), once the damping was already off. `nmpc_standstill_fade_speed` (default 3.0 m/s) now ramps the multiplier linearly back to 1x between `nmpc_standstill_speed` and that speed instead of switching it off in one tick; set it at or below `nmpc_standstill_speed` to restore the old hard cutoff. **Still open**: the fade is not yet live-tested, the default is off, the scale is a tuning value rather than a derived constant, and the separate launch speed overshoot (peaked 4.25 m/s against a 3.0 m/s target, accel saturating during the same dead window, and `v_desired` itself ramping 0.35 -> 18.0 m/s over 2.5 s) is not addressed by this change. |
