...
DEPENDENCIES:
  ...
  - utils.h: preprocess_img() (letterbox resize).
...
int Yolo::init(std::string engine_name) {
    ...
    */
    ...
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_input), batch_size * 3 * input_height * input_width * sizeof (float))); // Allocate GPU VRAM for the input tensor. Mirrors h_input in size.
    CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&d_output), batch_size * output_size * sizeof (float))); // Allocate GPU VRAM for the output tensor. TensorRT writes raw detections here after the forward pass.
    // Create stream
...
std::vector<BBoxInfo> Yolo::run(cv::Mat left_cv, int orig_image_h, int orig_image_w, float thres) {
    ...
    The zed_ros2_wrapper delivers images in BGRA format (bgra8 encoding, 4 channels with an
    Alpha layer), exactly as the ZED SDK did. preprocess_img() also performs letterbox
    resizing: it scales the image to fit within the network's input box while preserving
    the aspect ratio, then pads the empty space with grey (value 128) rather than
    stretching the image.
    */
    cv::Mat& left_cv_rgba = left_cv; // The incoming cv::Mat is already the BGRA frame (decoded by cv_bridge in the image callback).
    ...
    cv::Mat pr_img = preprocess_img(left_cv_rgb, input_width, input_height); // Letterbox resize to the network's exact input resolution (stays BGR — no colour conversion happens here). The returned image is input_width × input_height with grey padding on the shorter axis.

    /*
    Serialize the preprocessed OpenCV image (HWC layout, uint8, BGR) into the h_input float
    array (CHW layout, float32 in [0,1], RGB). The model is an Ultralytics YOLOv8 export and
    therefore expects RGB input (verified from the ONNX metadata_props). OpenCV stores pixels
    in HWC order with B,G,R interleaved per pixel. This nested loop performs three things at
    once: the HWC→CHW layout transpose, the BGR→RGB channel swap (by writing uc_pixel[2]
    first), and the uint8→float [0,1] normalization.
    */
