...
Subscribes to /fsae/perception/cone_detection (fsae_interfaces/ConeDetection) which
provides the car's pose and lists of blue/yellow cones in the camera's local frame.
...
Publishes:
  /fsae/slam/left_track  (fsae_interfaces/Track) - refined blue cone positions
  /fsae/slam/right_track (fsae_interfaces/Track) - refined yellow cone positions
  /fsae/slam/times_modified (Float32MultiArray)  - confidence scores of pruned cones (debug)

This is the working Python reference implementation (ported from the old repo's
reference_cone_landmark_mapper.py). The C++ node in src/ is a stub for a future
...
class Cone_Mapper(Node):
    def __init__(self, *vargs, **kwargs):
        ...
        self.left_track_publisher = self.create_publisher(Track, '/fsae/slam/left_track', 10)

        # Create right track publisher
        self.right_track_publisher = self.create_publisher(Track, '/fsae/slam/right_track', 10)

        # times_modified publisher
        self.times_modified_publisher = self.create_publisher(Float32MultiArray, '/fsae/slam/times_modified', 10)
        ...
        # Tuning parameters (extracted to fsae_bringup/config/fsae_params.yaml).
        self.declare_parameters(
            namespace='',
            parameters=[
                ('default_error_in_estimate', 4.0),  # Initial error in the estimate
                ('error_in_measurement', 2.5),       # Error in measurement
                ('match_radius', 1.0),               # Cone match radius
                ('remove_point_counter', 200),       # Counter to remove points
                ('times_modified_limit', 100),       # value for the times_modified
                ('modify_rate', 1.08),               # Modify rate
                ('reset_count', 60),                 # Detection reset counter
                ('invert_cones', False),
            ]
        )
        self.default_error_in_estimate = self.get_parameter('default_error_in_estimate').get_parameter_value().double_value
        self.error_in_measurement = self.get_parameter('error_in_measurement').get_parameter_value().double_value
        self.match_radius = self.get_parameter('match_radius').get_parameter_value().double_value
        self.remove_point_counter = self.get_parameter('remove_point_counter').get_parameter_value().integer_value
        self.times_modified_limit = self.get_parameter('times_modified_limit').get_parameter_value().integer_value
        self.modify_rate = self.get_parameter('modify_rate').get_parameter_value().double_value
        self.reset_count = self.get_parameter('reset_count').get_parameter_value().integer_value
    ...
    def cones_callback(self, msg: ConeDetection) -> None:
        """This function updates the existing cone map with newly detected cones and
        publishes left-track cones to the /fsae/slam/left_track topic and right-track cones to the /fsae/slam/right_track topic.

        Args:
            msg (ConeDetection): car's position and input cones from the /fsae/perception/cone_detection topic
        """
    ...
    def update_existing_cone_map(self, msg: ConeDetection) -> None:
        """This function updates the existing cone map using new cone measurements

        Args:
            msg (ConeDetection): Contains the car's position and lists of detected cones
        """
