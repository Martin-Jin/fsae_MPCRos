...
class StanleyControl(Node):
    ...
    def __init__(self):
        ...
        # Previously hardcoded as k_stanley/max_steer/cam_fron_axle_dist and the
        # 'vel' param. k_speed was declared but never used, so it is dropped.
        ...
        self.k_stanley = self.get_parameter('stanley_gain').get_parameter_value().double_value
        self.cam_fron_axle_dist = self.get_parameter('front_axle_dist').get_parameter_value().double_value
        self.max_steer = self.get_parameter('max_steer_angle').get_parameter_value().double_value
        # NOTE: faithful to old code - reads ParameterValue without .double_value here (see _notes known bugs).
        self.target_speed = self.get_parameter('target_velocity').get_parameter_value()
        ...
        self.create_subscription(PoseArray, "/fsae/planning/selected_trajectory", self.selected_trajectory_handler, 5)
        self.create_subscription(Pose, "/fsae/slam/car_position", self.main_hearback, 5)
        #Publish result
        ...
        # TODO Stage 4: this publisher's handle is discarded and never published to (dead in old code). Remove or wire up.
        self.create_publisher(Pose, "/fsae/control/track_point", 5)

    def main_hearback(self, msg):
        car_pose = msg
        camera_position = [car_pose.position.x,car_pose.position.y]
        car_yaw = car_pose.orientation.w
        # WARNING / TODO (re-tune): the `- 4.71` (~270 deg) was empirically tuned to cancel the
        # ZED's old non-ROS coordinate frame (RIGHT_HANDED_Z_UP, ~90 deg off). The ZED init was
        # FIXED to RIGHT_HANDED_Z_UP_X_FWD (true ROS axes) in zed_launch.cpp, so this constant is
        # now almost certainly WRONG and will steer ~90 deg off. With proper ROS yaw it should be
        # ~0 (i.e. `normalize_angle(car_yaw)`), but DO NOT trust that blind — re-tune and bench-test
        # this whole yaw block (also lines ~195/208/219) on the car before driving. Left unchanged
        # here because it can't be tested right now.
